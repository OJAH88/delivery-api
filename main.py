...
async def calculate_cart(request: Request):
    ...
    try:
        ...
        # 1. Fetch active promotions from DB
        ...
        for item in items:
            ...
            # 2. Check for bulk discount (trigger_qty_threshold)
            for p in promos:
                if item.get('category_id') == p.trigger_category_id and qty >= p.trigger_qty_threshold:
                    if p.reward_type == 'flat_discount':
                        total_discount += p.reward_value
        
        final_total = max(0, raw_total - total_discount)
        final_cash_total = round(final_cash_total / 5.0) * 5
